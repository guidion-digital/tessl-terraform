...
1. The skill is a **gate**. If the plan does not match your intent, the
   task is not done — investigate the mismatch; do not proceed with further
   edits or hand the task back to the user as finished.
2. Do not skip the skill because "the change looks trivial". Cosmetic edits
   frequently cause unintended `replace` actions on dependent resources.
3. If the AWS session check in the skill fails, stop and ask the user to
   authenticate. Do not attempt to work around it.
4. If you genuinely cannot run `terraform plan` in this environment (no
